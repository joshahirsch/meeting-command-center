...
| `DAILY_DIGEST_EMAIL` | No | `josh@nivahealth.com` |
| `DAILY_DIGEST_HOUR` | No | `8` (0–23, weekday trigger) |
| `DAILY_DIGEST_TIMEZONE` | No | `America/New_York` |
| `DAILY_DIGEST_OWNER_ALIASES` | No | `Josh,Josh Hirsch,josh@nivahealth.com` (comma-separated; digest only) |
| `GOOGLE_TASKS_LIST_NAME` | No | `Meeting Ops Actions` |
| `TASK_SYNC_ENABLED` | No | `false` |
...
| `sendDailyActionDigest` | Weekday email digest of Josh-owned open action items |
| `syncJoshActionItemsToGoogleTasks` | One-way sync eligible rows → Google Tasks |
| `installDailyDigestTrigger` | Daily weekday digest at `DAILY_DIGEST_HOUR` |
| `installGoogleTasksSyncTrigger` | Daily task sync one hour after digest hour |

Enable the **Tasks API** advanced service in the Apps Script editor (Services → Tasks API) after push; `appsscript.json` already declares it.
